...
import { Activity, ArrowRight, ArrowUpRight, Check, ChevronRight, Clock3, FileText, HeartPulse, Languages, Mail, MapPin, Menu, MessageCircle, Phone, Play, Send, ShieldCheck, Sparkles, Stethoscope, X } from 'lucide-react';
import { ErrorBoundary } from '@/components/error-boundary';
import { Toaster } from '@/components/ui/toaster';
import { TooltipProvider } from '@/components/ui/tooltip';
import { LanguageProvider, useLanguage } from '@/i18n/language';
import { NotFoundBody } from '@/pages/not-found';
...
function MobileActionBar() {
  ...
  return <div className="fixed inset-x-0 bottom-0 z-40 flex gap-2 border-t border-[hsl(var(--border))] bg-[hsl(var(--background)/.96)] p-3 backdrop-blur lg:hidden" data-testid="mobile-action-bar">
    <Link href="/referral" className="flex flex-1 items-center justify-center gap-2 rounded-full bg-[hsl(var(--primary))] px-4 py-3 text-xs font-bold text-[hsl(var(--primary-foreground))]" data-testid="link-mobile-refer">{t('Refer a patient', 'إحالة مريض')}</Link>
    {clinic?.whatsapp && <a href={`https://wa.me/${clinic.whatsapp.replace(/\D/g, '')}`} target="_blank" rel="noreferrer" className="flex items-center justify-center gap-2 rounded-full border border-[hsl(var(--border))] px-4 py-3 text-xs font-bold" data-testid="link-mobile-whatsapp"><MessageCircle size={15} /> {t('WhatsApp', 'واتساب')}</a>}
  </div>;
}

function Shell({ children }: { children: ReactNode }) {
  const { t } = useLanguage();
  return <div className="noise min-h-[100dvh] pb-20 lg:pb-0">
    <a href="#main" className="sr-only focus:not-sr-only focus:absolute focus:start-4 focus:top-4 focus:z-50 focus:rounded-full focus:bg-[hsl(var(--primary))] focus:px-5 focus:py-3 focus:text-sm focus:font-bold focus:text-[hsl(var(--primary-foreground))]" data-testid="link-skip-to-content">{t('Skip to content', 'تخطَّ إلى المحتوى')}</a>
    <Header />{children}<Footer /><MobileActionBar />
  </div>;
}
...
function Home() {
  const { t } = useLanguage();
  usePageMeta('', t('Prosthetic and orthotic mobility solutions from Mafaz, Amman.', 'حلول الأطراف الاصطناعية والأجهزة التقويمية من مفاز، عمّان.'));
  const query = useGetCatalogOverview({ query: { queryKey: getGetCatalogOverviewQueryKey(), staleTime: 300000 } });
  const overview = query.data;
  return <Shell><main id="main" className="page-in">
...
function CategoryPage() {
  ...
  const query = useGetCatalogCategory(slug, { query: { enabled: !!slug, queryKey: getGetCatalogCategoryQueryKey(slug), staleTime: 300000, retry: false } });
  const category = query.data;
  const isMissing = (query.error as { status?: number } | null)?.status === 404;
  usePageMeta(category ? t(category.title, category.titleArabic) : t('Catalog', 'الكتالوج'), category?.description);
  return <Shell><main id="main" className="page-in mx-auto max-w-7xl px-5 py-14 lg:px-10 lg:py-20">
    ...
    {query.isLoading ? <><div className="shimmer h-16 w-2/3 rounded-xl" /><div className="mt-5"><LoadingBlock /></div></> : isMissing ? <NotFoundBody heading={t('We have no such category.', 'لا يوجد لدينا هذا القسم.')} body={t('Prosthetics and orthotics are both in the catalog — start there.', 'الأطراف الاصطناعية والأجهزة التقويمية موجودة في الكتالوج — ابدأ من هناك.')} /> : query.isError || !category ? <ErrorBlock onRetry={() => query.refetch()} /> : <>
...
function NotFoundPage() { return <Shell><NotFoundBody /></Shell>; }

function SolutionDetail() { return <Shell><SolutionDetailBody /></Shell>; }

function Outcomes() {
  const { t } = useLanguage();
  usePageMeta(t('Clinical outcomes', 'النتائج السريرية'));
...
function Referral() {
  const { t } = useLanguage();
  usePageMeta(t('Refer a patient', 'إحالة مريض'), t('Send a patient referral to the Mafaz clinical team.', 'أرسل إحالة مريض إلى الفريق السريري في مفاز.'));
  ...
  /** Plain-text summary, used for WhatsApp and as the copyable fallback. */
  ...
  // The clinic's server sends the email, so a referrer needs no mail app.
  ...
  const whatsappHref = clinic?.whatsapp ? `https://wa.me/${clinic.whatsapp.replace(/\D/g, '')}` : null;

  const sendOnWhatsApp = (form: HTMLFormElement) => {
    setError('');
    if (!form.reportValidity()) return;
    if (!whatsappHref) { setError(t('The WhatsApp line is unavailable right now. Please call the clinic instead.', 'خط واتساب غير متاح حالياً. يرجى الاتصال بالعيادة.')); return; }
    const body = compose(collect(form));
    openExternal(`${whatsappHref}?text=${encodeURIComponent(body)}`, true);
    setSent({ delivered: false, body });
  };

  if (sent) return <Shell><main id="main" className="page-in mx-auto flex max-w-3xl flex-col items-center px-5 py-20 text-center lg:py-28">
    <div className={`grid size-20 place-items-center rounded-full ${sent.delivered ? 'bg-[hsl(var(--secondary)/.14)] text-[hsl(var(--secondary))]' : 'bg-[hsl(var(--accent)/.14)] text-[hsl(var(--accent))]'}`}><Check size={34} /></div>
    <SectionEyebrow>{sent.delivered ? t('Referral sent', 'تم إرسال الإحالة') : t('Almost there', 'أوشكت على الانتهاء')}</SectionEyebrow>
    <h1 className="font-serif text-6xl leading-[.9]">{sent.delivered ? t('It reached the team.', 'وصلت إلى الفريق.') : t('One last step.', 'خطوة أخيرة.')}</h1>
    <p className="mt-6 max-w-md text-base leading-7 text-[hsl(var(--muted-foreground))]" data-testid="text-referral-outcome">
      {sent.delivered
    ...
    {!sent.delivered && <section className="mt-10 w-full rounded-[1.4rem] border border-[hsl(var(--border))] bg-[hsl(var(--card)/.6)] p-5 text-start sm:p-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <p className="text-sm font-bold">{t('Send it yourself', 'أرسلها بنفسك')}</p>
      ...
      <div className="mt-4 flex flex-wrap gap-3">
        {whatsappHref && <a href={`${whatsappHref}?text=${encodeURIComponent(sent.body)}`} target="_blank" rel="noreferrer" className="flex items-center gap-2 rounded-full bg-[hsl(var(--primary))] px-4 py-2.5 text-xs font-bold text-[hsl(var(--primary-foreground))]" data-testid="link-confirm-whatsapp"><MessageCircle size={14} /> {t('Send on WhatsApp', 'أرسل عبر واتساب')}</a>}
        {clinic?.email && <a href={`mailto:${clinic.email}?subject=${encodeURIComponent(t('Patient referral', 'إحالة مريض'))}&body=${encodeURIComponent(sent.body)}`} dir="ltr" className="flex items-center gap-2 rounded-full border border-[hsl(var(--border))] px-4 py-2.5 text-xs font-bold" data-testid="link-confirm-email"><Mail size={14} /> {clinic.email}</a>}
      </div>
    </section>}
